...
- Viewerだけを全幅で表示するモード
- Quiet Lightを含むMermaid配色
- 本文フォントとViewer配色の切り替え
...
ホストされたデモを使う場合、ページを読み込む通信自体はホストへ発生します。
厳密なオフライン利用では、`dist/mermaid-markdown-local.html`をダウンロードし、
必要に応じてネットワークを切ったうえでローカルファイルとして開いてください。
...
`v0.1.0`のようなタグをpushすると、Releaseワークフローがビルドと検査を行い、
